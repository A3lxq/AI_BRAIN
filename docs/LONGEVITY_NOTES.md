# AI_BRAIN — Long-Term Viability Notes

- **Date:** 2026-08-24
- **Author:** Claude Code (AI_BRAIN Phase 0)
- **Purpose:** A standing record of the long-term-viability reasoning behind AI_BRAIN's Phase 0 technology choices — for the maintainer today, and for any future maintainer (including a future version of the current maintainer) inheriting this project years from now.
...
- **The hand-rolled Git automation module** (ADR-0005) — by not depending on GitPython, AI_BRAIN is immune to that specific project's decline. The cost is more code to write and maintain directly; the payoff is that this code cannot rot because someone else's dependency rotted.
- **The hand-rolled RAG and SQLite-access layers** (ADR-0003, ADR-0004) — same trade-off. More of AI_BRAIN's logic is code the project owns outright, which is more maintenance surface today but carries zero dependency-abandonment risk later.
