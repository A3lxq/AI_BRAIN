...
- **Distinct from ADR-0005's `.gitleaks.toml` allowlist.** No shared state between the two — one governs "safe to commit to ATHENA AI-BRAIN's own code repo," the other governs "safe to index as vault content."
...
**MCP surface:** two small tools following ADR-0007's existing contract shape: `secret_findings_list(status="flagged")` (read-only) and `secret_finding_resolve(finding_id, resolution: "allowlist"|"acknowledge", reason)` (mutating, touches only ATHENA AI-BRAIN's own metadata, not vault content — MRTR-confirmation classification belongs to ADR-0007's owners). `vault_status`/`note_provenance` should also surface `secret_scan_status` per-note, consistent with `index_state`.
...
- Fail-open would only be defensible if this control were advisory/best-effort. It isn't — `docs/SECURITY_MODEL.md`'s checklist places it in **P0**, "must be resolved before any real vault is pointed at ATHENA AI-BRAIN."
