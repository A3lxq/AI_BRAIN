...
|---|---|---|
| `athena.db` | `chunks.chunk_text` (full chunk bodies, FTS5-indexed), `notes.title`, provenance/lifecycle metadata | ADR-0004 |
...
**Called from:** ADR-0004's migration runner, once per file, before the first connection is opened to `athena.db`, and before ATHENA AI-BRAIN's bootstrap constructs `SqliteHuey(filename=...)` (since Huey's own constructor is what causes its `.db` file to be created if absent — the helper must run first, or Huey must be pointed at a path this helper has already pre-created).
...
1. **Fresh-creation mode assertion:** run the migration runner against a clean temp directory; assert `athena.db`'s resulting `stat().st_mode & 0o777 == 0o600`, not the OS/umask default (`0o644`).
