# ADR-0008: Embeddings, Sparse-Vector, and Reranker Model Choice for ATHENA AI-BRAIN

- **ID:** ADR-0008
- **Title:** Embeddings, Sparse-Vector, and Reranker Model Choice for ATHENA AI-BRAIN
- **Status:** Accepted
...
| Option | Verdict |
|---|---|
| Qwen3-Embedding-8B | Rejected as primary — oversized for ATHENA AI-BRAIN's corpus scale; 4096-dim would inflate Qdrant storage ~4x for benchmark-scale quality gains unlikely to manifest at ATHENA AI-BRAIN's actual (low tens of thousands of chunks) scale. |
...
| Per-content-type embedding models (titles/tags vs. bodies) | Rejected — this pattern is justified mainly at commercial search volume; unjustified complexity at ATHENA AI-BRAIN's personal-vault scale per "do not optimize prematurely." |

## Rationale

1. **BGE-M3 is the most battle-tested production choice** among viable candidates, with a mature reranker pairing and permissive MIT license, while running comfortably on CPU at ATHENA AI-BRAIN's scale without requiring a GPU.
...
3. **miniCOIL's English-only limitation is treated as an operational risk to monitor, not ignored** — a documented fallback (Qdrant's BM25-based sparse indexing) exists precisely because ATHENA AI-BRAIN's constitution requires evidence-based decisions, and the actual language composition of a given user's vault isn't known until Phase 1 measurement.
...
- Original chunk text must be retained independent of Qdrant-stored vectors (e.g., in ATHENA AI-BRAIN's SQLite metadata store per ADR-0004), enabling re-embedding without needing to re-derive text from the vault on every model swap.
