...
The ATHENA AI-BRAIN software repository is separate from the Obsidian knowledge vault repository.
...
ATHENA AI-BRAIN may automate commits and pushes, but automation must:
- be configurable,
- detect failures,
...
The sections above state the principles and policy. These runbooks make them concrete and actionable against ATHENA AI-BRAIN's accepted architecture (ADR-0002, ADR-0005, ADR-0006, ADR-0007).
...
8. **Post-upgrade cleanup**: after a soak period (e.g. a few days of normal operation), retire the pre-upgrade snapshot per ATHENA AI-BRAIN's backup retention policy — keep at least one prior snapshot generation as a safety margin, don't delete immediately after a successful upgrade.
...
- **False-positive handling specific to ATHENA AI-BRAIN's domain**: vault notes and test fixtures may legitimately contain secret-shaped strings (example API keys in docs, planted fake-secret test fixtures per the testing strategy's security test methodology). Handle via gitleaks' inline allowlist mechanism or tightly-scoped fixture-path exclusions, reviewed at PR time — never broad path exclusions.
- **ATHENA AI-BRAIN's own automated commit path** (below) must treat a gitleaks block as a first-class, visibly-surfaced failure mode, not a silent failure to commit.

### ATHENA AI-BRAIN's own automated Git commit/push policy
...
### Branching policy (ATHENA AI-BRAIN's own software repository)

Governs the ATHENA AI-BRAIN codebase repository — distinct from the vault repository's auto-commit/push policy above.
