...
The Phase 3 counterpart to `ai_brain.vault.ingest.ingest_note` -- chained
after that job's success (see `ai_brain.worker`), never called standalone
...
from ai_brain.indexing.chunking import chunk_note
from ai_brain.indexing.embedding import EMBEDDING_MODEL_VERSION, embed_dense, embed_sparse
from ai_brain.indexing.qdrant_store import delete_points_for_note, upsert_chunks
...
from ai_brain.safety.paths import PathMode, VaultRoot, resolve_vault_path
from ai_brain.security.secrets import redact_high_confidence_spans, scan_note_for_secrets
...
async def index_bootstrap(
    ...
    *,
    ...
    """`ai-brain index bootstrap` (design doc §6): a one-time pass indexing
    ...
    chaining in `ai_brain.worker` would otherwise ever revisit."""
